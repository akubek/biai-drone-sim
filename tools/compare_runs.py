...
class RunSummary:
    ...
    generations: int
    evaluations: int
    jitter_best: float          # srednia |zmiana| best_fitness miedzy generacjami
    jitter_mean: float
    final_best: float
    final_success: float
    final_crash: float
...
def print_table(runs: list[RunSummary]) -> None:
    cols = [
        ...
        ("skok best", lambda r: f"{r.jitter_best:.1f}", 10),
        ("skok mean", lambda r: f"{r.jitter_mean:.2f}", 10),
    ...
    print("-" * len(header))
    for run in runs:
        print("  ".join(f"{str(fn(run)):<{width}}" for _, fn, width in cols))
